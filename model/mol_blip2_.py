...
class BertSelfAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, is_2d=True):
        super().__init__()
        self.config = config
        ...
        if is_cross_attention:
            if is_2d:
                self.key = nn.Linear(config.d2_encoder_width, self.all_head_size)
                self.value = nn.Linear(config.d2_encoder_width, self.all_head_size)
            else: 
                self.key = nn.Linear(config.d3_encoder_width, self.all_head_size)
                self.value = nn.Linear(config.d3_encoder_width, self.all_head_size)
        else:
    ...
    def forward(
        ...
        # and values come from an encoder; the attention mask neexds to be
...
class BertAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, is_2d=True):
        super().__init__()
        self.self = BertSelfAttention(config, is_cross_attention, is_2d)
...
class BertLayer(nn.Module):
    def __init__(self, config, layer_num):
        ...
        if (
            ...
            self.crossattention_2d = BertAttention(
                config, is_cross_attention=self.config.add_cross_attention, is_2d=True
            )
            self.crossattention_3d = BertAttention(
                config, is_cross_attention=self.config.add_cross_attention, is_2d=False
            )
    ...
    def forward(
        ...
        if query_length > 0:
            ...
            if self.has_cross_attention:
                ...
                cross_attention = self.crossattention_2d if is_2d else self.crossattention_3d
  #              print(f'??: {cross_attention}') #2d, 3d일때 서로 다른 6개의 cross-attention layer 통과! 
                cross_attention_outputs = cross_attention(
    ...
    def forward(
        ...
        for i in range(self.config.num_hidden_layers):
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                #2d/3d well separated for use_cache = True
                ...
                #well-working for when use_cache = False
...
class MolBlip2Base(BaseModel):
    ...
    @classmethod
    def init_Qformer(self, model_name, num_query_token, d2_graph_width, d3_graph_width, cross_attention_freq=2):
        assert model_name == 'scibert'
        if True:
            bert_name = 'allenai/scibert_scivocab_uncased'
        
        tokenizer = AutoTokenizer.from_pretrained(bert_name, cache_dir='./ckpts/SciBert')
        tokenizer.add_special_tokens({"bos_token": "[DEC]"})

        encoder_config = BertConfig.from_pretrained(bert_name)
        encoder_config.d2_encoder_width = d2_graph_width
        encoder_config.d3_encoder_width = d3_graph_width
