...
from model.mol_blip2_llama_ import MolBlip2Llama
...
class Blip2Stage2(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log("molecule loss", float(loss['loss']), batch_size=batch_size, sync_dist=True, on_epoch=True) ###########################################################################################
        self.log("lr", self.trainer.optimizers[0].param_groups[0]['lr'], batch_size=batch_size, sync_dist=True, on_epoch=True) ###########################################################################################
        return loss['loss']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        loss = self.blip2opt(batch)
        ###============== Overall Loss ===================###
        self.log("val molecule loss", float(loss['loss']), batch_size=batch_size, sync_dist=True, on_epoch=True) ###########################################################################################
        return loss['loss']

    @torch.no_grad()
    def test_step(self, batch, batch_idx):
        d3_graphs, prompt_tokens, d2_graphs, texts = batch
        ###============== Captioning Results ===================###
        samples = {'graphs': (d3_graphs, d2_graphs), 'prompt_tokens': prompt_tokens}
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument('--caption_eval_epoch', type=int, default=2)
